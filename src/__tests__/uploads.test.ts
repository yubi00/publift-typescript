...
import app from '../app';
import { store } from '../routes/router';
import { data } from './fixtures/data';

test('Should upload a csv file', async (done) => {
  const filePath = path.join(__dirname, 'fixtures', 'analytics.csv');
  await request(app).post('/api/uploads').attach('file', filePath).expect(200);
